...
HMODULE CHexEdit::GetCurrentModule(BOOL bRef/* = FALSE*/)
{
...
void CHexEdit::GetHexString(TCHAR* sBuffer, BYTE* lpBuffer, int nCount)
{
	...
	int needkonglen = (m_nBytesPerRow - nCount) * 3;
	for (int i = 0; i < needkonglen; i++)
	{
		*sBuffer++ = ' ';
	}
#else
...
{
	//AFX_MANAGE_STATE(AfxGetStaticModuleState());//dll中使用需要添加
